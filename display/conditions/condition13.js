/**
 * Condition 13: Visual Offset Bug
 * Based on Condition 4: Ensemble + Hover, but with visual offset bug
 * Aggregated by default, hover to reveal alternatives with 10px offset (bug)
 */
import { ChartRenderer } from '../base/chartRenderer.js';
import { InteractionManager } from '../base/interactionManager.js';

export default class Condition13 {
    ...
    render() {
        ...
        // Render alternative prediction lines (hidden initially, with offset bug)
    ...
    renderAlternativeLines(predictionGroup) {
        ...
        // BUG: Apply 20px vertical offset to alternative lines
        this.alternativesGroupA = predictionGroup.append("g")
            .attr("class", "alternatives-group-a")
            .attr("transform", "translate(0, 20)") // BUG: 20px vertical offset
            .style("opacity", 0);
    ...
    renderAggregatedLines(predictionGroup) {
        // Use the base renderer for dashed aggregated lines (no offset)
