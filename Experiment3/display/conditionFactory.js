...
export class ConditionFactory {
    ...
    async createCondition(conditionNumber, svgId, conditionSpec = null) {
        try {
            ...
            const condition = new module.default(svgId, this.processedData, this.config, this.phase, conditionSpec);
     ...
     */
    async renderCondition(conditionNumber, svgId, conditionSpec = null) {
        try {
            const condition = await this.createCondition(conditionNumber, svgId, conditionSpec);
    ...
    async renderAllConditions() {
        const conditionMappings = [
            ...
            { number: 18, svgId: 'chart-13' },  // Glitch Ensemble + Hover
            { number: 19, svgId: 'chart-14' },  // Glitch PI Plot + Hover
            { number: 20, svgId: 'chart-15' },  // Glitch PI → Ensemble
    ...
    static getConditionInfo() {
        return {
            ...
            5: { name: 'PI Plot + Hover', description: 'PI plot with hover to reveal confidence bounds' },
            ...
            18: { name: 'Glitch Ensemble + Hover', description: 'Ensemble + hover with millisecond-gated alternative-line visibility using a hidden set of 30 time buckets' },
            19: { name: 'Glitch PI Plot + Hover', description: 'PI plot + hover with millisecond-gated confidence-bound visibility using a hidden set of 30 time buckets' },
            20: { name: 'Glitch PI → Ensemble', description: 'PI → ensemble transformation with millisecond-gated hover visibility using a hidden set of 30 time buckets' },
